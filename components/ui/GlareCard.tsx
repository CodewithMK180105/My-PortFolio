...
//   const backgroundStyle: CustomCSSProperties = {
//     "--step": "5%",
...
// // CustomCSSProperties: Extending CSSProperties for custom variables
// interface CustomCSSProperties extends React.CSSProperties {
...
//       <div
//         className={cn(
//           "relative grid h-full origin-center overflow-hidden rounded-[var(--radius)] border border-slate-800 bg-slate-950",
//           className
//         )}
//       >
//         {children}
...
// CustomCSSProperties: Extending CSSProperties to allow custom CSS variables
interface CustomCSSProperties extends React.CSSProperties {
  [key: `--${string}`]: string | number | undefined;
}
...
export const GlareCard = ({
  ...
  return (
    ...
    >
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-800 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden">
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))]">
          <div className={cn("h-full w-full bg-slate-950", className)}>
            {children}
          </div>
        ...
        />
        <div
          className="w-full h-full grid [grid-area:1/1] mix-blend-color-dodge opacity-[var(--opacity)] [background-blend-mode:hue_hue_hue_overlay] [background:var(--pattern),_var(--rainbow),_var(--diagonal),_var(--shade)] relative"
          style={{ backgroundBlendMode: "hue, hue, hue, overlay" }}
        />
      </div>
    </div>
  );
